...
async function seedPermissions() {
    ...
    console.log(`  Permissions: ${permissions_1.PERMISSION_DEFINITIONS.length}`);
}
async function seedRoles() {
    ...
    let grants = 0;
    for (const definition of roles_1.ROLE_DEFINITIONS) {
        ...
        for (const key of (0, roles_1.permissionsForRole)(definition)) {
            const permissionId = permissionIdByKey.get(key);
            if (!permissionId) {
                throw new Error(`Role "${definition.name}" references unknown permission key "${key}". ` +
    ...
    console.log(`  Roles: ${roles_1.ROLE_DEFINITIONS.length} with ${grants} permission grants`);
}
