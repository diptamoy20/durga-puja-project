...
function validateEnv(config) {
    ...
    if (!databaseUrl) {
        ...
        errors.push('DATABASE_URL still contains the CHANGE_ME placeholder. Set your PostgreSQL password in backend/.env.');
    }
    ...
    if (secret && secret === refreshSecret) {
        errors.push('JWT_SECRET and JWT_REFRESH_SECRET must differ, otherwise a refresh token can be replayed as an access token.');
    }
