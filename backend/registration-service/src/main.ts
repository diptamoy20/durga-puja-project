...
async function bootstrap(): Promise<void> {
  const logger = new Logger('RegistrationService');
  ...
  const host = config.get<string>('registration.host') ?? 'localhost';
  const port = config.get<number>('registration.port') ?? 5003;
